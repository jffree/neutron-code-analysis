...
* `neutron.api.extension` 找到 `plugin_aware_extension_middleware_factory`

* 在 `neutron.api.extension` 找到 `ExtensionMiddleware`

* 在 `ExtensionMiddleware` 的 `__init__` 方法中利用 `routes.Mapper` 实现了 extension 路由映射， `mapper` 变量中存储的即是路由映射内容。

我们在 `__init__` 方法的最后一行增加以下代码来实现调试：
...
* 接下来，我们找到 `ExtensionMiddleware` 的 `__call__` 方法，我们发现这里依然是使用了 `routes.middleware.RoutesMiddleware` 中间件来实现访问 extension 的路由分发（由访问路径到处理方法的调用）。

* 接着向下，我们找到了 `_dispatch` 方法，这里就实现了对请求的处理。我们就在这个 `_dispatch` 方法的第一行增加一条调试语句：｀LOG.info('========================== ExtensionMiddleware._dispatch: %s ' % req.environ)｀。 这条语句可以让我们看到法向 neutron server 的 wsgi 请求的 environ 变量内容。

* 我们用 curl 来访问一下 neutron 来测试一下：
